...
define('JOOMDEV_WSS_BITLY_API', 'http://api.bit.ly/v3/'); 
define('JOOMDEV_WSS_BITLY_OAUTH_API', 'https://api-ssl.bit.ly/v3/'); 
define('JOOMDEV_WSS_BITLY_OAUTH_ACCESS_TOKEN', 'https://api-ssl.bit.ly/oauth/');

if(!class_exists('JoomdevWssBitly')) {

  class JoomdevWssBitly{

    function bitly_oauth_access_token($code, $redirect, $client_id, $client_secret) {
      $results = array();
      $url = JOOMDEV_WSS_BITLY_OAUTH_ACCESS_TOKEN . "access_token";
      $params = array();
    ...
    function bitly_get($endpoint, $params, $complex=false) {
      ...
      if ($complex) {
        ...
        $url = JOOMDEV_WSS_BITLY_OAUTH_API . $endpoint . "?" . substr($url_params, 1);
      } else {
        $url = JOOMDEV_WSS_BITLY_OAUTH_API . $endpoint . "?" . http_build_query($params);
      }
    ...
    function bitly_post($endpoint, $params) {
      $result = array();
      $url = JOOMDEV_WSS_BITLY_OAUTH_API . $api_endpoint;
