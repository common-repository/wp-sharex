...
function get_joomdev_wss_options(){
	$default = array(
				'networks' => array('facebook' => 'Facebook', 'twitter' => 'Twitter', 'googleplus' => 'Google Plus', 'yahoomail' => 'Yahoo Mail', 'linkedin' => 'Linkedin'),
				'button_locations' => array('bottom_of_content'),
				'display_buttons_in' => array('post'),
				'url_shorting' => 'bit.ly',
				'bit_ly_username' => '',
				'bit_ly_access_token' => '',
				'goo_gl_api_key' => '',
				...
				'share_incentive' => '',
				'hide_on_mobile_top_of_the_content' => 'no',
				'hide_on_mobile_bottom_of_the_content' => 'no',
				'hide_on_mobile_sidebar' => 'no',
				'hide_on_mobile_media' => 'no',
			);
			...
			if(is_single() || is_singular()){
				if(in_array(get_post_type($post->ID), $display_in)){
					...
					if(is_array($locations) && in_array('media', $locations)){
						add_filter('the_content', 'joomdev_wss_generate_share_buttons_wrap_for_media', 999);
						add_filter('the_content', 'joomdev_wss_generate_share_buttons_media', 999);
						// add_action('wp_footer', 'joomdev_wss_generate_share_buttons_media_footer', 999);
					}
				}
			}
		}
...
function joomdev_wss_get_short_url($options, $url = ''){
	...
	if($url){
		...
		if($options['url_shorting'] == 'goo.gl'){
			include_once 'lib/joomdev-wss-googleurl.php';
			$goo_gl_api_key = $options['goo_gl_api_key'];

			$GoogleUrlApi = new JoomdevWssGoogleUrlApi($goo_gl_api_key);
	...
	if(!empty($selected_networks)){
        ...
        <div class="joomdev-wss-social-share-buttons joomdev-wss-social-share-buttons-top">
            <?php 
					...
					// }
            echo '<div class="jd-share-wrapper">';
        ...
        <div class="joomdev-wss-social-share-buttons joomdev-wss-social-share-buttons-sidebar">
            <div class="joomdev-wss-social-share-buttons-sidebar-inner-container">
                ...
                <?php 
					...
					if($n >= 5){
						echo '<div class="joomdev-wss-social-share-button joomdev-wss-social-share-'.$joomdev_wss_options['buttons_format'].' joomdev-share-more-button">';
							?>
...
function joomdev_wss_generate_share_buttons_wrap_for_media($content){
	ob_start();

	?>
		<div class="joomdev-wss-share_buttons_wrap_for_media">
			<?php echo $content; ?>
		</div>
	<?php 
...
function joomdev_wss_generate_share_buttons_media($content){
	global $post;
	...
	$pattern = '/(<img[^>]*class=\"([^>]*?)\"[^>]*>)/i';
   	$replacement = '<div class="joomdev-wss-social-share-buttons-media-wrapper">'.$network_html.'$1</div>';
   	$content = preg_replace($pattern, $replacement, $content);

   	return $content;
}

function joomdev_wss_generate_share_buttons_media_footer(){
	/*global $post;
	// echo 'media';
	...
	if(!empty($selected_networks)){
        ...
        <script type="text/javascript">
            jQuery(function($) {
                $(document).find('.joomdev-wss-share_buttons_wrap_for_media img').each(function() {
